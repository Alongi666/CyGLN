...
#     def __init__(self, in_dim, out_dim, feat_drop=0.2, attn_drop=0.2, gnn='rgat_r'):
...
#         # equation (4)
#         h = self.feat_drop(torch.sum(alpha * (nodes.mailbox['z'] + nodes.mailbox['r_h']), dim=1))
#         h += torch.mm(nodes.data['z'], self.loop_weight)  # Apply self-loop
#         return {'h': h}
#
...
#
#         # Optionally update edge features if required
#         if edge_update:
#             # Edge feature update logic can be added here
#             pass
#
#         # equation (2)
...
class TimeEncode(torch.nn.Module):
    ...
    def forward(self, pos):
        map_pos = pos.unsqueeze(1) * self.basis_freq
        map_pos += self.phase
        # 将 sin 和 cos 进行拼接
        pos_enc = torch.cat([torch.sin(map_pos), torch.cos(map_pos)], dim=-1)
        return pos_enc
...
#         # 添加衰减率作为可学习参数
#         self.alpha = nn.Parameter(torch.ones(1).float())
#
#     def forward(self, pos):
#         # 计算衰减因子，假设pos是1-D张量
#         decay_factor = torch.exp(-self.alpha * pos)
#         # 将衰减因子应用到所有的基础频率上
#         map_pos = pos.unsqueeze(1) * self.basis_freq * decay_factor.unsqueeze(1)
#         map_pos += self.phase
#         # 将 sin 和 cos 进行拼接
#         pos_enc = torch.cat([torch.sin(map_pos), torch.cos(map_pos)], dim=-1)
#         return pos_enc

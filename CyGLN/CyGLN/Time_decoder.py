...
class TimeConvTransR(torch.nn.Module):
    def __init__(self, num_relations, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=40, kernel_size=3, use_bias=True):
    ...
    def forward(self, embedding, emb_rel, emb_time, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        ...
        if partial_embeding is None:
            ...
            x = torch.mm(x, emb_rel.transpose(1, 0))
            x = x.to('cuda')
            partial_embeding = partial_embeding.to('cuda')
            x = torch.mul(x, partial_embeding)
        return x


class TimeConvTransE(torch.nn.Module):
    def __init__(self, num_entities, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=40, kernel_size=3, use_bias=True):
